...
void run_context_tests(int use_prealloc) {
	...
	if (use_prealloc) {
		none_prealloc = malloc(secp256k1_context_preallocated_size(
		    SECP256K1_CONTEXT_NONE));
		sign_prealloc = malloc(secp256k1_context_preallocated_size(
		    SECP256K1_CONTEXT_SIGN));
		vrfy_prealloc = malloc(secp256k1_context_preallocated_size(
		    SECP256K1_CONTEXT_VERIFY));
		both_prealloc = malloc(secp256k1_context_preallocated_size(
		    SECP256K1_CONTEXT_SIGN | SECP256K1_CONTEXT_VERIFY));
		CHECK(none_prealloc != NULL);
		CHECK(sign_prealloc != NULL);
		CHECK(vrfy_prealloc != NULL);
		CHECK(both_prealloc != NULL);
		none = secp256k1_context_preallocated_create(
		    none_prealloc, SECP256K1_CONTEXT_NONE);
		sign = secp256k1_context_preallocated_create(
		    sign_prealloc, SECP256K1_CONTEXT_SIGN);
		vrfy = secp256k1_context_preallocated_create(
		    vrfy_prealloc, SECP256K1_CONTEXT_VERIFY);
		both = secp256k1_context_preallocated_create(
		    both_prealloc,
		    SECP256K1_CONTEXT_SIGN | SECP256K1_CONTEXT_VERIFY);
	} else {
	...
	secp256k1_context_set_illegal_callback(
	    vrfy, counting_illegal_callback_fn, &ecount);
	secp256k1_context_set_illegal_callback(
	    sign, counting_illegal_callback_fn, &ecount2);
	secp256k1_context_set_error_callback(sign, counting_illegal_callback_fn,
					     NULL);
	CHECK(vrfy->error_callback.fn != sign->error_callback.fn);

	/* check if sizes for cloning are consistent */
	CHECK(secp256k1_context_preallocated_clone_size(none) ==
	      secp256k1_context_preallocated_size(SECP256K1_CONTEXT_NONE));
	CHECK(secp256k1_context_preallocated_clone_size(sign) ==
	      secp256k1_context_preallocated_size(SECP256K1_CONTEXT_SIGN));
	CHECK(secp256k1_context_preallocated_clone_size(vrfy) ==
	      secp256k1_context_preallocated_size(SECP256K1_CONTEXT_VERIFY));
	CHECK(secp256k1_context_preallocated_clone_size(both) ==
	      secp256k1_context_preallocated_size(SECP256K1_CONTEXT_SIGN |
						  SECP256K1_CONTEXT_VERIFY));
	 ...
	 * ***/
	{
		...
		if (use_prealloc) {
			/* clone into a non-preallocated context and then again
			 * into a new preallocated one. */
			ctx_tmp = none;
			none = secp256k1_context_clone(none);
			secp256k1_context_preallocated_destroy(ctx_tmp);
			free(none_prealloc);
			none_prealloc =
			    malloc(secp256k1_context_preallocated_size(
				SECP256K1_CONTEXT_NONE));
			CHECK(none_prealloc != NULL);
			ctx_tmp = none;
			none = secp256k1_context_preallocated_clone(
			    none, none_prealloc);
			...
			CHECK(sign_prealloc != NULL);
			ctx_tmp = sign;
			sign = secp256k1_context_preallocated_clone(
			    sign, sign_prealloc);
			secp256k1_context_destroy(ctx_tmp);
			...
			CHECK(vrfy_prealloc != NULL);
			ctx_tmp = vrfy;
			vrfy = secp256k1_context_preallocated_clone(
			    vrfy, vrfy_prealloc);
			secp256k1_context_destroy(ctx_tmp);
			...
			CHECK(both_prealloc != NULL);
			ctx_tmp = both;
			both = secp256k1_context_preallocated_clone(
			    both, both_prealloc);
			secp256k1_context_destroy(ctx_tmp);
		} else {
			/* clone into a preallocated context and then again into
			 * a new non-preallocated one. */
			void *prealloc_tmp;
	...
	} while (!secp256k1_ecdsa_sig_sign(&both->ecmult_gen_ctx, &sigr, &sigs,
					   &key, &msg, &nonce, NULL));

	/* try signing */
	CHECK(secp256k1_ecdsa_sig_sign(&sign->ecmult_gen_ctx, &sigr, &sigs,
				       &key, &msg, &nonce, NULL));
	CHECK(secp256k1_ecdsa_sig_sign(&both->ecmult_gen_ctx, &sigr, &sigs,
				       &key, &msg, &nonce, NULL));

	/* try verifying */
	CHECK(secp256k1_ecdsa_sig_verify(&vrfy->ecmult_ctx, &sigr, &sigs, &pub,
					 &msg));
	CHECK(secp256k1_ecdsa_sig_verify(&both->ecmult_ctx, &sigr, &sigs, &pub,
					 &msg));
...
void run_scratch_tests(void) {
	...
	/* Test public API */
	secp256k1_context_set_illegal_callback(
	    none, counting_illegal_callback_fn, &ecount);
	...
	CHECK(secp256k1_scratch_max_allocation(scratch, 1) <
	      500); /* 500 - ALIGNMENT */
	CHECK(secp256k1_scratch_alloc(scratch, 500) != NULL);
	CHECK(secp256k1_scratch_alloc(scratch, 500) == NULL);
...
void run_sha256_tests(void) {
	...
	static const unsigned char outputs[8][32] = {
	    ...
	    {0xf3, 0x0c, 0xeb, 0x2b, 0xb2, 0x82, 0x9e, 0x79, 0xe4, 0xca, 0x97,
	     0x53, 0xd3, 0x5a, 0x8e, 0xcc, 0x00, 0x26, 0x2d, 0x16, 0x4c, 0xc0,
	     0x77, 0x08, 0x02, 0x95, 0x38, 0x1c, 0xbd, 0x64, 0x3f, 0x0d},
	    {0x68, 0x19, 0xd9, 0x15, 0xc7, 0x3f, 0x4d, 0x1e, 0x77, 0xe4, 0xe1,
	     0xb5, 0x2d, 0x1f, 0xa0, 0xf9, 0xcf, 0x9b, 0xea, 0xea, 0xd3, 0x93,
	     0x9f, 0x15, 0x87, 0x4b, 0xd9, 0x88, 0xe2, 0xa2, 0x36, 0x30},
	    {0x24, 0x8d, 0x6a, 0x61, 0xd2, 0x06, 0x38, 0xb8, 0xe5, 0xc0, 0x26,
	     0x93, 0x0c, 0x3e, 0x60, 0x39, 0xa3, 0x3c, 0xe4, 0x59, 0x64, 0xff,
	     0x21, 0x67, 0xf6, 0xec, 0xed, 0xd4, 0x19, 0xdb, 0x06, 0xc1},
	    {0xf0, 0x8a, 0x78, 0xcb, 0xba, 0xee, 0x08, 0x2b, 0x05, 0x2a, 0xe0,
	     0x70, 0x8f, 0x32, 0xfa, 0x1e, 0x50, 0xc5, 0xc4, 0x21, 0xaa, 0x77,
	     0x2b, 0xa5, 0xdb, 0xb4, 0x06, 0xa2, 0xea, 0x6b, 0xe3, 0x42},
	    {0xab, 0x64, 0xef, 0xf7, 0xe8, 0x8e, 0x2e, 0x46, 0x16, 0x5e, 0x29,
	     0xf2, 0xbc, 0xe4, 0x18, 0x26, 0xbd, 0x4c, 0x7b, 0x35, 0x52, 0xf6,
	     0xb3, 0x82, 0xa9, 0xe7, 0xd3, 0xaf, 0x47, 0xc2, 0x45, 0xf8}};
	int i;
	for (i = 0; i < 8; i++) {
		unsigned char out[32];
		secp256k1_sha256 hasher;
		secp256k1_sha256_initialize(&hasher);
		secp256k1_sha256_write(&hasher,
		...
		if (strlen(inputs[i]) > 0) {
			...
			secp256k1_sha256_write(
			    &hasher, (const unsigned char *)(inputs[i]), split);
			secp256k1_sha256_write(
			    &hasher, (const unsigned char *)(inputs[i] + split),
...
void run_hmac_sha256_tests(void) {
	...
	for (i = 0; i < 6; i++) {
		...
		secp256k1_hmac_sha256_initialize(
		    &hasher, (const unsigned char *)(keys[i]), strlen(keys[i]));
		...
		if (strlen(inputs[i]) > 0) {
			int split = secp256k1_rand_int(strlen(inputs[i]));
			secp256k1_hmac_sha256_initialize(
			    &hasher, (const unsigned char *)(keys[i]),
			    strlen(keys[i]));
...
void run_rfc6979_hmac_sha256_tests(void) {
	static const unsigned char key1[65] = {
	    ...
	    0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x00, 0x4b,
	    0xf5, 0x12, 0x2f, 0x34, 0x45, 0x54, 0xc5, 0x3b, 0xde, 0x2e, 0xbb,
	    0x8c, 0xd2, 0xb7, 0xe3, 0xd1, 0x60, 0x0a, 0xd6, 0x31, 0xc3, 0x85,
	    0xa5, 0xd7, 0xcc, 0xe2, 0x3c, 0x77, 0x85, 0x45, 0x9a, 0};
	static const unsigned char out1[3][32] = {
	    {0x4f, 0xe2, 0x95, 0x25, 0xb2, 0x08, 0x68, 0x09, 0x15, 0x9a, 0xcd,
	     0xf0, 0x50, 0x6e, 0xfb, 0x86, 0xb0, 0xec, 0x93, 0x2c, 0x7b, 0xa4,
	     0x42, 0x56, 0xab, 0x32, 0x1e, 0x42, 0x1e, 0x67, 0xe9, 0xfb},
	    {0x2b, 0xf0, 0xff, 0xf1, 0xd3, 0xc3, 0x78, 0xa2, 0x2d, 0xc5, 0xde,
	     0x1d, 0x85, 0x65, 0x22, 0x32, 0x5c, 0x65, 0xb5, 0x04, 0x49, 0x1a,
	     0x0c, 0xbd, 0x01, 0xcb, 0x8f, 0x3a, 0xa6, 0x7f, 0xfd, 0x4a},
	    {0xf5, 0x28, 0xb4, 0x10, 0xcb, 0x54, 0x1f, 0x77, 0x00, 0x0d, 0x7a,
	     0xfb, 0x6c, 0x5b, 0x53, 0xc5, 0xc4, 0x71, 0xea, 0xb4, 0x3e, 0x46,
	     0x6d, 0x9a, 0xc5, 0x19, 0x0c, 0x39, 0xc8, 0x2f, 0xd8, 0x2e}};
	...
	static const unsigned char out2[3][32] = {
	    {0x9c, 0x23, 0x6c, 0x16, 0x5b, 0x82, 0xae, 0x0c, 0xd5, 0x90, 0x65,
	     0x9e, 0x10, 0x0b, 0x6b, 0xab, 0x30, 0x36, 0xe7, 0xba, 0x8b, 0x06,
	     0x74, 0x9b, 0xaf, 0x69, 0x81, 0xe1, 0x6f, 0x1a, 0x2b, 0x95},
	    {0xdf, 0x47, 0x10, 0x61, 0x62, 0x5b, 0xc0, 0xea, 0x14, 0xb6, 0x82,
	     0xfe, 0xee, 0x2c, 0x9c, 0x02, 0xf2, 0x35, 0xda, 0x04, 0x20, 0x4c,
	     0x1d, 0x62, 0xa1, 0x53, 0x6c, 0x6e, 0x17, 0xae, 0xd7, 0xa9},
	    {0x75, 0x97, 0x88, 0x7c, 0xbd, 0x76, 0x32, 0x1f, 0x32, 0xe3, 0x04,
	     0x40, 0x67, 0x9a, 0x22, 0xcf, 0x7f, 0x8d, 0x9d, 0x2e, 0xac, 0x39,
	     0x0e, 0x58, 0x1f, 0xea, 0x09, 0x1c, 0xe2, 0x02, 0xba, 0x94}};
...
void test_rand_bits(int rand32, int bits) {
	/* (1-1/2^B)^rounds[B] < 1/10^9, so rounds is the number of iterations
	 * to get a false negative chance below once in a billion */
	static const unsigned int rounds[7] = {1, 30, 73, 156, 322, 653, 1316};
	/* We try multiplying the results with various odd numbers, which
	 * shouldn't influence the uniform distribution modulo a power of 2. */
	...
	for (i = 0; i < rounds[usebits]; i++) {
		uint32_t r =
		    (rand32 ? secp256k1_rand32() : secp256k1_rand_bits(bits));
...
void test_num_jacobi(void) {
	...
	CHECK(secp256k1_num_jacobi(&n, &order) ==
	      1); /* sage confirms this is 1 */
}
...
void scalar_test(void) {
	...
	/* Set 's2' to a random scalar, with value 'snum2', and byte array
	 * representation 'c'. */
	...
	{
		int i;
		/* Test that fetching groups of 4 bits from a scalar and
		 * recursing n(i)=16*n(i-1)+p(i) reconstructs it. */
		secp256k1_scalar n;
		...
		for (i = 0; i < 256; i += 4) {
			...
			secp256k1_scalar_set_int(
			    &t, secp256k1_scalar_get_bits(&s, 256 - 4 - i, 4));
			for (j = 0; j < 4; j++) {
	...
	{
		/* Test that fetching groups of randomly-sized bits from a
		 * scalar and recursing n(i)=b*n(i-1)+p(i) reconstructs it. */
		secp256k1_scalar n;
		int i = 0;
		secp256k1_scalar_set_int(&n, 0);
		while (i < 256) {
			...
			secp256k1_scalar_set_int(
			    &t, secp256k1_scalar_get_bits_var(&s, 256 - now - i,
							      now));
			for (j = 0; j < now; j++) {
				secp256k1_scalar_add(&n, &n, &n);
			}
...
#ifndef USE_NUM_NONE
	{
		/* Test that adding the scalars together is equal to adding
		 * their numbers together modulo the order. */
		secp256k1_num rnum;
		secp256k1_num r2num;
		secp256k1_scalar r;
	...
	{
		/* Test that multiplying the scalars is equal to multiplying
		 * their numbers modulo the order. */
		secp256k1_scalar r;
		secp256k1_num r2num;
		secp256k1_num rnum;
		...
		secp256k1_scalar_get_num(&r2num, &r);
		CHECK(secp256k1_num_eq(&rnum, &r2num));
		/* The result can only be zero if at least one of the factors
		 * was zero. */
		CHECK(secp256k1_scalar_is_zero(&r) ==
		      (secp256k1_scalar_is_zero(&s) ||
		       secp256k1_scalar_is_zero(&s2)));
		/* The results can only be equal to one of the factors if that
		 * factor was zero, or the other factor was one. */
		CHECK(secp256k1_num_eq(&rnum, &snum) ==
		      (secp256k1_scalar_is_zero(&s) ||
		       secp256k1_scalar_is_one(&s2)));
		CHECK(secp256k1_num_eq(&rnum, &s2num) ==
		      (secp256k1_scalar_is_zero(&s2) ||
...

	{
		...
		/* Check that comparison with zero matches comparison with zero
		 * on the number. */
		CHECK(secp256k1_num_is_zero(&snum) ==
		      secp256k1_scalar_is_zero(&s));
		/* Check that comparison with the half order is equal to testing
		 * for high scalar. */
		CHECK(secp256k1_scalar_is_high(&s) ==
		      (secp256k1_num_cmp(&snum, &half_order) > 0));
		secp256k1_scalar_negate(&neg, &s);
		secp256k1_num_sub(&negnum, &order, &snum);
		secp256k1_num_mod(&negnum, &order);
		/* Check that comparison with the half order is equal to testing
		 * for high scalar after negation. */
		CHECK(secp256k1_scalar_is_high(&neg) ==
		      (secp256k1_num_cmp(&negnum, &half_order) > 0));
		/* Negating should change the high property, unless the value
		 * was already zero. */
		CHECK((secp256k1_scalar_is_high(&s) ==
		       secp256k1_scalar_is_high(&neg)) ==
		      secp256k1_scalar_is_zero(&s));
		secp256k1_scalar_get_num(&negnum2, &neg);
		/* Negating a scalar should be equal to (order - n) mod order on
		 * the number. */
	...
	{
		/* Test that scalar inverses are equal to the inverse of their
		 * number modulo the order. */
		if (!secp256k1_scalar_is_zero(&s)) {
			...
			/* Multiplying a scalar with its inverse must result in
			 * one. */
...
void scalar_chacha_tests(void) {
	...
	unsigned char expected3[64] = {
	    0x47, 0x4a, 0x4f, 0x35, 0x4f, 0xee, 0x93, 0x59, 0xbb, 0x65, 0x81,
	    0xe5, 0xd9, 0x15, 0xa6, 0x01, 0xb6, 0x8c, 0x68, 0x03, 0x38, 0xff,
	    0x65, 0xe6, 0x56, 0x4a, 0x3e, 0x65, 0x59, 0xfc, 0x12, 0x3f, 0xa9,
	    0xb2, 0xf9, 0x3e, 0x57, 0xc3, 0xa5, 0xcb, 0xe0, 0x72, 0x74, 0x27,
	    0x88, 0x1c, 0x23, 0xdf, 0xe2, 0xb6, 0xcc, 0xfb, 0x93, 0xed, 0xcb,
	    0x02, 0xd7, 0x50, 0x52, 0x45, 0x84, 0x88, 0xbb, 0xea};
...
void run_scalar_tests(void) {
...

	{
		...
		static const secp256k1_scalar overflowed =
	...
	{
		/* Static test vectors.
		 * These were reduced from ~10^12 random vectors based on
		 * comparison-decision and edge-case coverage on 32-bit and
		 * 64-bit implementations. The responses were generated with
		 * Sage 5.9.
		 */
		secp256k1_scalar x;
		secp256k1_scalar y;
...
void run_field_convert(void) {
	static const unsigned char b32[32] = {
	    0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x11, 0x12, 0x13,
...
/* This compares jacobian points including their Z, not just their geometric
 * meaning. */
...
void test_ge(void) {
	...
	int runs = 4;
#endif
	/* Points: (infinity, p1, p1, -p1, -p1, p2, p2, -p2, -p2, p3, p3, -p3,
	 * -p3, p4, p4, -p4, -p4). The second in each pair of identical points
	 * uses a random Z coordinate in the Jacobian form. All magnitudes are
	 * randomized. All 17*17 combinations of points are added to each other,
	 * using all applicable methods.
	 *
	 * When the endomorphism code is compiled in, p5 = lambda*p1 and p6 =
	 * lambda^2*p1 are added as well.
	 */
	secp256k1_ge *ge = (secp256k1_ge *)checked_malloc(
	    &ctx->error_callback, sizeof(secp256k1_ge) * (1 + 4 * runs));
	secp256k1_gej *gej = (secp256k1_gej *)checked_malloc(
	    &ctx->error_callback, sizeof(secp256k1_gej) * (1 + 4 * runs));
	secp256k1_fe *zinv = (secp256k1_fe *)checked_malloc(
	    &ctx->error_callback, sizeof(secp256k1_fe) * (1 + 4 * runs));
	secp256k1_fe zf;
	...
	for (i = 0; i < runs; i++) {
		...
		random_group_element_jacobian_test(&gej[2 + 4 * i],
						   &ge[2 + 4 * i]);
		secp256k1_gej_set_ge(&gej[3 + 4 * i], &ge[3 + 4 * i]);
		random_group_element_jacobian_test(&gej[4 + 4 * i],
						   &ge[4 + 4 * i]);
		for (j = 0; j < 4; j++) {
	...
	{
		secp256k1_fe *zs =
		    checked_malloc(&ctx->error_callback,
				   sizeof(secp256k1_fe) * (1 + 4 * runs));
		for (i = 0; i < 4 * runs + 1; i++) {
			if (i == 0) {
				/* The point at infinity does not have a
				 * meaningful z inverse. Any should do. */
				do {
	...
	for (i1 = 0; i1 < 1 + 4 * runs; i1++) {
		...
		for (i2 = 0; i2 < 1 + 4 * runs; i2++) {
			...
			secp256k1_gej_add_var(
			    &refj, &gej[i1], &gej[i2],
			    secp256k1_gej_is_infinity(&gej[i1]) ? NULL : &zr);
			/* Check Z ratio. */
			if (!secp256k1_gej_is_infinity(&gej[i1]) &&
			    !secp256k1_gej_is_infinity(&refj)) {
				secp256k1_fe zrz;
			...
			if (!secp256k1_gej_is_infinity(&gej[i1]) &&
			    !secp256k1_gej_is_infinity(&resj)) {
				secp256k1_fe zrz;
				secp256k1_fe_mul(&zrz, &zr, &gej[i1].z);
			...
			/* Test gej + ge (var, with additional Z factor). */
			{
				secp256k1_ge ge2_zfi =
				    ge[i2]; /* the second term with x and y
					       rescaled for z = 1/zf */
				...
				secp256k1_gej_add_zinv_var(&resj, &gej[i1],
							   &ge2_zfi, &zf);
				ge_equals_gej(&ref, &resj);
			}
			...
			/* Test gej + ge (const). */
			if (i2 != 0) {
				/* secp256k1_gej_add_ge does not support its
				 * second argument being infinity. */
	...
	/* Test adding all points together in random order equals infinity. */
	{
		...
		secp256k1_gej *gej_shuffled = (secp256k1_gej *)checked_malloc(
		    &ctx->error_callback,
		    (4 * runs + 1) * sizeof(secp256k1_gej));
		for (i = 0; i < 4 * runs + 1; i++) {
	...
	/* Test batch gej -> ge conversion with and without known z ratios. */
	{
		secp256k1_fe *zr = (secp256k1_fe *)checked_malloc(
		    &ctx->error_callback,
		    (4 * runs + 1) * sizeof(secp256k1_fe));
		secp256k1_ge *ge_set_all = (secp256k1_ge *)checked_malloc(
		    &ctx->error_callback,
		    (4 * runs + 1) * sizeof(secp256k1_ge));
		for (i = 0; i < 4 * runs + 1; i++) {
			/* Compute gej[i + 1].z / gez[i].z (with gej[n].z taken
			 * to be 1). */
			if (i < 4 * runs) {
				secp256k1_fe_mul(&zr[i + 1], &zinv[i],
						 &gej[i + 1].z);
			}
		}
...
void test_add_neg_y_diff_x(void) {
	 ...
	 * G =
	 * C.lift_x(0x79BE667EF9DCBBAC55A06295CE870B07029BFCDB2DCE28D959F2815B16F81798)
	 ...
	 */
	secp256k1_gej aj =
	    SECP256K1_GEJ_CONST(0x8d24cd95, 0x0a355af1, 0x3c543505, 0x44238d30,
	...
	secp256k1_gej bj =
	    SECP256K1_GEJ_CONST(0xc7b74206, 0x1f788cd9, 0xabd0937d, 0x164a0d86,
	...
	secp256k1_gej sumj =
	    SECP256K1_GEJ_CONST(0x671a63c0, 0x3efdad4c, 0x389a7798, 0x24356027,
...
void run_ecmult_chain(void) {
	...
	secp256k1_gej a =
	    SECP256K1_GEJ_CONST(0x8b30bbe9, 0xae2a9906, 0x96b22f67, 0x0709dff3,
	...
	secp256k1_scalar xn = SECP256K1_SCALAR_CONST(
	    0x84cc5452, 0xf7fde1ed, 0xb4d38a8c, 0xe9b1b84c, 0xcef31f14,
	    0x6e569be9, 0x705d357a, 0x42985407);
	secp256k1_scalar gn = SECP256K1_SCALAR_CONST(
	    0xa1e58d22, 0x553dcd42, 0xb2398062, 0x5d4c57a9, 0x6e9323d4,
	    0x2b3152e5, 0xca2c3990, 0xedc7c9de);
	/* two small multipliers to be applied to xn and gn in every iteration:
	 */
	static const secp256k1_scalar xf =
	    SECP256K1_SCALAR_CONST(0, 0, 0, 0, 0, 0, 0, 0x1337);
	static const secp256k1_scalar gf =
	    SECP256K1_SCALAR_CONST(0, 0, 0, 0, 0, 0, 0, 0x7113);
	...
	for (i = 0; i < 200 * count; i++) {
		...
		/* also compute ae and ge: the actual accumulated factors for A
		 * and G */
		/* if X was (ae*A+ge*G), xn*X + gn*G results in (xn*ae*A +
		 * (xn*ge+gn)*G) */
	...
	/* redo the computation, but directly with the resulting ae and ge
	 * coefficients: */
...
void test_point_times_order(const secp256k1_gej *point) {
	...
	secp256k1_ecmult(&ctx->ecmult_ctx, &res1, point, &x,
			 &x); /* calc res1 = x * point + x * G; */
	secp256k1_ecmult(
	    &ctx->ecmult_ctx, &res2, point, &nx,
	    &nx); /* calc res2 = (order - x) * point + (order - x) * G; */
...
void run_point_times_order(void) {
	...
	static const secp256k1_fe xr =
	    SECP256K1_FE_CONST(0x7603CB59, 0xB0EF6C63, 0xFE608479, 0x2A0C378C,
...
void ecmult_const_random_mult(void) {
	/* random starting point A (on the curve) */
	secp256k1_ge a =
	    SECP256K1_GE_CONST(0x6d986544, 0x57ff52b8, 0xcf1b8126, 0x5b802a5b,
	...
	secp256k1_scalar xn = SECP256K1_SCALAR_CONST(
	    0x649d4f77, 0xc4242df7, 0x7f2079c9, 0x14530327, 0xa31b876a,
	    0xd2d8ce2a, 0x2236d5c6, 0xd7b2029b);
	/* expected xn * A (from sage) */
	secp256k1_ge expected_b =
	    SECP256K1_GE_CONST(0x23773684, 0x4d209dc7, 0x098a786f, 0x20d06fcd,
...
void ecmult_const_chain_multiply(void) {
	...
	const secp256k1_scalar scalar = SECP256K1_SCALAR_CONST(
	    0x4968d524, 0x2abf9b7a, 0x466abbcf, 0x34b11b6d, 0xcd83d307,
	    0x827bed62, 0x05fad0ce, 0x18fae63b);
	const secp256k1_gej expected_point =
	    SECP256K1_GEJ_CONST(0x5494c15d, 0x32099706, 0xc2395f94, 0x348745fd,
...
static int ecmult_multi_callback(secp256k1_scalar *sc, secp256k1_ge *pt,
...
static int ecmult_multi_false_callback(secp256k1_scalar *sc, secp256k1_ge *pt,
				       size_t idx, void *cbdata) {
	(void)sc;
	(void)pt;
	(void)idx;
...
void test_ecmult_multi(secp256k1_scratch *scratch,
	...
	CHECK(ecmult_multi(&ctx->ecmult_ctx, scratch, &r, NULL,
	...
	for (ncount = 0; ncount < count; ncount++) {
		...
		CHECK(ecmult_multi(&ctx->ecmult_ctx, scratch, &r, &sc[0],
				   ecmult_multi_callback, &data, 0));
		...
		CHECK(ecmult_multi(&ctx->ecmult_ctx, scratch, &r, &szero,
		...
		scratch_empty =
		    secp256k1_scratch_create(&ctx->error_callback, 0);
		CHECK(!ecmult_multi(&ctx->ecmult_ctx, scratch_empty, &r, &szero,
				    ecmult_multi_callback, &data, 1));
		...
		secp256k1_ecmult(&ctx->ecmult_ctx, &r2, &ptgj, &sc[0], &sc[1]);
		CHECK(ecmult_multi(&ctx->ecmult_ctx, scratch, &r, &sc[1],
				   ecmult_multi_callback, &data, 1));
		secp256k1_gej_neg(&r2, &r2);
		secp256k1_gej_add_var(&r, &r, &r2, NULL);
		CHECK(secp256k1_gej_is_infinity(&r));
	...
	for (ncount = 0; ncount < count; ncount++) {
		...
		for (j = 0; j < 3; j++) {
			...
			CHECK(ecmult_multi(&ctx->ecmult_ctx, scratch, &r,
					   &szero, ecmult_multi_callback, &data,
					   sizes[j]));
			CHECK(secp256k1_gej_is_infinity(&r));
		}
		...
		for (j = 0; j < 3; j++) {
			...
			for (i = 0; i < 16; i++) {
				...
				secp256k1_scalar_negate(&sc[2 * i + 1],
							&sc[2 * i]);
				pt[2 * i] = ptg;
				pt[2 * i + 1] = ptg;
			}
	...
	/* Run through s0*(t0*P) + s1*(t1*P) exhaustively for many small values
	 * of s0, s1, t0, t1 */
	{
		const size_t TOP = 8;
		size_t s0i, s1i;
		...
		for (t0i = 0; t0i < TOP; t0i++) {
			for (t1i = 0; t1i < TOP; t1i++) {
				...
				secp256k1_ecmult(&ctx->ecmult_ctx, &t0p, &ptgj,
						 &t0, &szero);
				secp256k1_ecmult(&ctx->ecmult_ctx, &t1p, &ptgj,
						 &t1, &szero);
				...
				for (s0i = 0; s0i < TOP; s0i++) {
					for (s1i = 0; s1i < TOP; s1i++) {
						...
						secp256k1_ge_set_gej(&pt[0],
								     &t0p);
						secp256k1_ge_set_gej(&pt[1],
								     &t1p);
						...
						secp256k1_scalar_mul(&tmp1, &t0,
								     &sc[0]);
						secp256k1_scalar_mul(&tmp2, &t1,
								     &sc[1]);
						secp256k1_scalar_add(
						    &tmp1, &tmp1, &tmp2);

						secp256k1_ecmult(
						    &ctx->ecmult_ctx, &expected,
						    &ptgj, &tmp1, &szero);
						CHECK(ecmult_multi(
						    &ctx->ecmult_ctx, scratch,
						    &actual, &szero,
						    ecmult_multi_callback,
						    &data, 2));
						secp256k1_gej_neg(&expected,
								  &expected);
						secp256k1_gej_add_var(
						    &actual, &actual, &expected,
						    NULL);
						CHECK(secp256k1_gej_is_infinity(
						    &actual));
					}
				}
			}
...
#endif
		CHECK(secp256k1_pippenger_bucket_window(
			  secp256k1_pippenger_bucket_window_inv(i)) == i);
		if (i != PIPPENGER_MAX_BUCKET_WINDOW) {
			CHECK(secp256k1_pippenger_bucket_window(
				  secp256k1_pippenger_bucket_window_inv(i) +
				  1) > i);
		}
	}
}
...
/**
 * Probabilistically test the function returning the maximum number of possible
 * points for a given scratch space.
 */
void test_ecmult_multi_pippenger_max_points(void) {
	size_t scratch_size = secp256k1_rand_int(256);
	size_t max_size = secp256k1_pippenger_scratch_size(
	    secp256k1_pippenger_bucket_window_inv(PIPPENGER_MAX_BUCKET_WINDOW -
						  1) +
		512,
	    12);
	...
	for (; scratch_size < max_size; scratch_size += 256) {
		...
		bucket_window =
		    secp256k1_pippenger_bucket_window(n_points_supported);
		CHECK(secp256k1_scratch_allocate_frame(
		    scratch,
		    secp256k1_pippenger_scratch_size(n_points_supported,
						     bucket_window),
...
/**
 * Run secp256k1_ecmult_multi_var with num points and a scratch space restricted
 * to 1 <= i <= num points.
 */
void test_ecmult_multi_batching(void) {
	...
	secp256k1_scalar *sc = (secp256k1_scalar *)checked_malloc(
	    &ctx->error_callback, sizeof(secp256k1_scalar) * n_points);
	secp256k1_ge *pt = (secp256k1_ge *)checked_malloc(
	    &ctx->error_callback, sizeof(secp256k1_ge) * n_points);
	secp256k1_gej r;
	secp256k1_gej r2;
	ecmult_multi_data data;
	int i;
	...
	CHECK(!secp256k1_ecmult_multi_var(&ctx->ecmult_ctx, scratch, &r, &scG,
					  ecmult_multi_callback, &data, 1));
	secp256k1_scratch_destroy(scratch);

	/* Test with space for 1 point in pippenger. That's not enough because
	 * ecmult_multi selects strauss which requires more memory. */
	scratch = secp256k1_scratch_create(
	    &ctx->error_callback, secp256k1_pippenger_scratch_size(1, 1) +
	...
	for (i = 1; i <= n_points; i++) {
		if (i > ECMULT_PIPPENGER_THRESHOLD) {
			int bucket_window =
			    secp256k1_pippenger_bucket_window(i);
			size_t scratch_size =
			    secp256k1_pippenger_scratch_size(i, bucket_window);
			scratch = secp256k1_scratch_create(
			    &ctx->error_callback,
			    scratch_size +
				PIPPENGER_SCRATCH_OBJECTS * ALIGNMENT);
			...
			scratch = secp256k1_scratch_create(
			    &ctx->error_callback,
			    scratch_size + STRAUSS_SCRATCH_OBJECTS * ALIGNMENT);
		}
		CHECK(secp256k1_ecmult_multi_var(&ctx->ecmult_ctx, scratch, &r,
						 &scG, ecmult_multi_callback,
						 &data, n_points));
		secp256k1_gej_add_var(&r, &r, &r2, NULL);
		CHECK(secp256k1_gej_is_infinity(&r));
		secp256k1_scratch_destroy(scratch);
...
void run_ecmult_multi_tests(void) {
	...
	scratch = secp256k1_scratch_create(
	    &ctx->error_callback, secp256k1_strauss_scratch_size(1) +
				      STRAUSS_SCRATCH_OBJECTS * ALIGNMENT);
	test_ecmult_multi(scratch, secp256k1_ecmult_multi_var);
	secp256k1_scratch_destroy(scratch);
...
void test_wnaf(const secp256k1_scalar *number, int w) {
	...
	for (i = bits - 1; i >= 0; i--) {
		...
		if (v) {
			CHECK(
			    zeroes == -1 ||
			    zeroes >=
				w - 1); /* check that distance between non-zero
					   elements is at least w-1 */
			zeroes = 0;
			CHECK((v & 1) ==
			      1); /* check non-zero elements are odd */
			CHECK(v <= (1 << (w - 1)) - 1);	 /* check range below */
			CHECK(v >= -(1 << (w - 1)) - 1); /* check range above */
		} else {
			CHECK(zeroes != -1); /* check that no unnecessary zero
						padding exists */
			zeroes++;
		}
		if (v >= 0) {
	...
	CHECK(secp256k1_scalar_eq(
	    &x, number)); /* check that wnaf represents number */
}
...
void test_constant_wnaf_negate(const secp256k1_scalar *number) {
	...
	sign2 = secp256k1_scalar_cond_negate(&neg2,
					     secp256k1_scalar_is_even(&neg2));
	CHECK(sign1 == sign2);
...
void run_wnaf(void) {
	...
	/* Sanity check: 1 and 2 are the smallest odd and even numbers and
	 * should have easier-to-diagnose failure modes  */
	n.d[0] = 1;
...
void test_ecmult_gen_blind(void) {
	/* Test ecmult_gen() blinding and confirm that the blinding changes, the
	 * affine points match, and the z's don't match. */
...
void test_ecmult_gen_blind_reset(void) {
	/* Test ecmult_gen() blinding reset and confirm that the blinding is
	 * consistent. */
	secp256k1_scalar b;
...
void ec_pubkey_parse_pointtest(const unsigned char *input, int xvalid,
			       int yvalid) {
	...
	ecount = 0;
	secp256k1_context_set_illegal_callback(
	    ctx, counting_illegal_callback_fn, &ecount);
	for (pubkeyclen = 3; pubkeyclen <= 65; pubkeyclen++) {
		...
		for (i = 0; i < 256; i++) {
			...
			ysign = (input[63] & 1) + 2;
			/* For the current type (i) do we expect parsing to
			 * work? Handled all of compressed/uncompressed/hybrid.
			 */
			xpass =
			    xvalid && (pubkeyclen == 33) && ((i & 254) == 2);
			/* Do we expect a parse and re-serialize as uncompressed
			 * to give a matching y? */
			ypass = xvalid && yvalid &&
				((i & 4) == ((pubkeyclen == 65) << 2)) &&
				((i == 4) || ((i & 251) == ysign)) &&
			...
			if (xpass || ypass) {
				...
				VG_UNDEF(&pubkey, sizeof(pubkey));
				ecount = 0;
				CHECK(secp256k1_ec_pubkey_parse(
					  ctx, &pubkey, pubkeyc, pubkeyclen) ==
				      1);
				VG_CHECK(&pubkey, sizeof(pubkey));
				outl = 65;
				VG_UNDEF(pubkeyo, 65);
				CHECK(secp256k1_ec_pubkey_serialize(
					  ctx, pubkeyo, &outl, &pubkey,
					  SECP256K1_EC_COMPRESSED) == 1);
				VG_CHECK(pubkeyo, outl);
				CHECK(outl == 33);
				CHECK(memcmp(&pubkeyo[1], &pubkeyc[1], 32) ==
				      0);
				CHECK((pubkeyclen != 33) ||
				      (pubkeyo[0] == pubkeyc[0]));
				if (ypass) {
					/* This test isn't always done because
					 * we decode with alternative signs, so
					 * the y won't match. */
					CHECK(pubkeyo[0] == ysign);
					CHECK(secp256k1_pubkey_load(
						  ctx, &ge, &pubkey) == 1);
					memset(&pubkey, 0, sizeof(pubkey));
					VG_UNDEF(&pubkey, sizeof(pubkey));
					...
					CHECK(outl == 65);
					CHECK(pubkeyo[0] == 4);
					CHECK(memcmp(&pubkeyo[1], input, 64) ==
					      0);
				}
				CHECK(ecount == 0);
			} else {
				...
				VG_CHECK(&pubkey, sizeof(pubkey));
				CHECK(ecount == 0);
				CHECK(secp256k1_pubkey_load(ctx, &ge,
							    &pubkey) == 0);
				CHECK(ecount == 1);
			}
		}
...
#define SECP256K1_EC_PARSE_TEST_NVALID (12)
	const unsigned char valid[SECP256K1_EC_PARSE_TEST_NVALID][64] = {
	    {/* Point with leading and trailing zeros in x and y serialization.
	     ...
	     0x00, 0x00, 0x00, 0x42, 0x52, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	     0x00, 0x64, 0xef, 0xa1, 0x7b, 0x77, 0x61, 0xe1, 0xe4, 0x27, 0x06,
	     0x98, 0x9f, 0xb4, 0x83, 0xb8, 0xd2, 0xd4, 0x9b, 0xf7, 0x8f, 0xae,
	     0x98, 0x03, 0xf0, 0x99, 0xb8, 0x34, 0xed, 0xeb, 0x00},
	    {
		/* Point with x equal to a 3rd root of unity.*/
		0x7a, 0xe9, 0x6a, 0x2b, 0x65, 0x7c, 0x07, 0x10, 0x6e, 0x64,
		0x47, 0x9e, 0xac, 0x34, 0x34, 0xe9, 0x9c, 0xf0, 0x49, 0x75,
		0x12, 0xf5, 0x89, 0x95, 0xc1, 0x39, 0x6c, 0x28, 0x71, 0x95,
		0x01, 0xee, 0x42, 0x18, 0xf2, 0x0a, 0xe6, 0xc6, 0x46, 0xb3,
		0x63, 0xdb, 0x68, 0x60, 0x58, 0x22, 0xfb, 0x14, 0x26, 0x4c,
		0xa8, 0xd2, 0x58, 0x7f, 0xdd, 0x6f, 0xbc, 0x75, 0x0d, 0x58,
		0x7e, 0x76, 0xa7, 0xee,
	    },
	    {
		...
		0xfc, 0x2c, 0x0e, 0x99, 0x4b, 0x14, 0xea, 0x72, 0xf8, 0xc3,
		0xeb, 0x95, 0xc7, 0x1e, 0xf6, 0x92, 0x57, 0x5e, 0x77, 0x50,
		0x58, 0x33, 0x2d, 0x7e, 0x52, 0xd0, 0x99, 0x5c, 0xf8, 0x03,
		0x88, 0x71, 0xb6, 0x7d,
	    },
	    {
		...
		0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
		0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe, 0xff, 0xff,
		0xfc, 0x2c, 0xf1, 0x66, 0xb4, 0xeb, 0x15, 0x8d, 0x07, 0x3c,
		0x14, 0x6a, 0x38, 0xe1, 0x09, 0x6d, 0xa8, 0xa1, 0x88, 0xaf,
		0xa7, 0xcc, 0xd2, 0x81, 0xad, 0x2f, 0x66, 0xa3, 0x07, 0xfb,
		0x77, 0x8e, 0x45, 0xb2,
	    },
	    {
		...
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x01, 0x42, 0x18, 0xf2, 0x0a, 0xe6, 0xc6, 0x46, 0xb3,
		0x63, 0xdb, 0x68, 0x60, 0x58, 0x22, 0xfb, 0x14, 0x26, 0x4c,
		0xa8, 0xd2, 0x58, 0x7f, 0xdd, 0x6f, 0xbc, 0x75, 0x0d, 0x58,
		0x7e, 0x76, 0xa7, 0xee,
	    },
		...
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x01, 0xbd, 0xe7, 0x0d, 0xf5, 0x19, 0x39, 0xb9, 0x4c,
		0x9c, 0x24, 0x97, 0x9f, 0xa7, 0xdd, 0x04, 0xeb, 0xd9, 0xb3,
		0x57, 0x2d, 0xa7, 0x80, 0x22, 0x90, 0x43, 0x8a, 0xf2, 0xa6,
		0x81, 0x89, 0x54, 0x41,
	    },
	    {
		/* Point with largest y. (1/3) */
		0x1f, 0xe1, 0xe5, 0xef, 0x3f, 0xce, 0xb5, 0xc1, 0x35, 0xab,
		0x77, 0x41, 0x33, 0x3c, 0xe5, 0xa6, 0xe8, 0x0d, 0x68, 0x16,
		0x76, 0x53, 0xf6, 0xb2, 0xb2, 0x4b, 0xcb, 0xcf, 0xaa, 0xaf,
		...
		0xcb, 0xb0, 0xde, 0xab, 0x12, 0x57, 0x54, 0xf1, 0xfd, 0xb2,
		0x03, 0x8b, 0x04, 0x34, 0xed, 0x9c, 0xb3, 0xfb, 0x53, 0xab,
		0x73, 0x53, 0x91, 0x12, 0x99, 0x94, 0xa5, 0x35, 0xd9, 0x25,
		0xf6, 0x73, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
		0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
		0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe,
		0xff, 0xff, 0xfc, 0x2e,
	    },
		...
		0x14, 0x6d, 0x3b, 0x65, 0xad, 0xd9, 0xf5, 0x4c, 0xcc, 0xa2,
		0x85, 0x33, 0xc8, 0x8e, 0x2c, 0xbc, 0x63, 0xf7, 0x44, 0x3e,
		0x16, 0x58, 0x78, 0x3a, 0xb4, 0x1f, 0x8e, 0xf9, 0x7c, 0x2a,
		0x10, 0xb5, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
		0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
		0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe,
		0xff, 0xff, 0xfc, 0x2e,
	    },
		...
		0x77, 0x41, 0x33, 0x3c, 0xe5, 0xa6, 0xe8, 0x0d, 0x68, 0x16,
		0x76, 0x53, 0xf6, 0xb2, 0xb2, 0x4b, 0xcb, 0xcf, 0xaa, 0xaf,
		0xf5, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x01,
	    },
		...
		0x03, 0x8b, 0x04, 0x34, 0xed, 0x9c, 0xb3, 0xfb, 0x53, 0xab,
		0x73, 0x53, 0x91, 0x12, 0x99, 0x94, 0xa5, 0x35, 0xd9, 0x25,
		0xf6, 0x73, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x01,
	    },
	    {/* Point with smallest y. (3/3) */
	     0x14, 0x6d, 0x3b, 0x65, 0xad, 0xd9, 0xf5, 0x4c, 0xcc, 0xa2, 0x85,
	     0x33, 0xc8, 0x8e, 0x2c, 0xbc, 0x63, 0xf7, 0x44, 0x3e, 0x16, 0x58,
	     0x78, 0x3a, 0xb4, 0x1f, 0x8e, 0xf9, 0x7c, 0x2a, 0x10, 0xb5, 0x00,
	     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
#define SECP256K1_EC_PARSE_TEST_NINVALID (7)
	const unsigned char invalid[SECP256K1_EC_PARSE_TEST_NINVALID][64] = {
	    {
		/* x is third root of -8, y is -1 * (x^3+7); also on the curve
		   for y^2 = x^3 + 9. */
		0x0a, 0x2d, 0x2b, 0xa9, 0x35, 0x07, 0xf1, 0xdf, 0x23, 0x37,
		0x70, 0xc2, 0xa7, 0x97, 0x96, 0x2c, 0xc6, 0x1f, 0x6d, 0x15,
		0xda, 0x14, 0xec, 0xd4, 0x7d, 0x8d, 0x27, 0xae, 0x1c, 0xd5,
		0xf8, 0x53, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x01,
	    },
		...
		0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
		0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe, 0xff, 0xff,
		0xfc, 0x30, 0x42, 0x18, 0xf2, 0x0a, 0xe6, 0xc6, 0x46, 0xb3,
		0x63, 0xdb, 0x68, 0x60, 0x58, 0x22, 0xfb, 0x14, 0x26, 0x4c,
		0xa8, 0xd2, 0x58, 0x7f, 0xdd, 0x6f, 0xbc, 0x75, 0x0d, 0x58,
		0x7e, 0x76, 0xa7, 0xee,
	    },
		...
		0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
		0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe, 0xff, 0xff,
		0xfc, 0x30, 0xbd, 0xe7, 0x0d, 0xf5, 0x19, 0x39, 0xb9, 0x4c,
		0x9c, 0x24, 0x97, 0x9f, 0xa7, 0xdd, 0x04, 0xeb, 0xd9, 0xb3,
		0x57, 0x2d, 0xa7, 0x80, 0x22, 0x90, 0x43, 0x8a, 0xf2, 0xa6,
		0x81, 0x89, 0x54, 0x41,
	    },
	    {
		/* x is -1, y is the result of the sqrt ladder; also on the
		   curve for y^2 = x^3 - 5. */
		0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
		0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
		0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe, 0xff, 0xff,
		0xfc, 0x2e, 0xf4, 0x84, 0x14, 0x5c, 0xb0, 0x14, 0x9b, 0x82,
		0x5d, 0xff, 0x41, 0x2f, 0xa0, 0x52, 0xa8, 0x3f, 0xcb, 0x72,
		0xdb, 0x61, 0xd5, 0x6f, 0x37, 0x70, 0xce, 0x06, 0x6b, 0x73,
		0x49, 0xa2, 0xaa, 0x28,
	    },
	    {
		/* x is -1, y is the result of the sqrt ladder; also on the
		...
		0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
		0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe, 0xff, 0xff,
		0xfc, 0x2e, 0x0b, 0x7b, 0xeb, 0xa3, 0x4f, 0xeb, 0x64, 0x7d,
		0xa2, 0x00, 0xbe, 0xd0, 0x5f, 0xad, 0x57, 0xc0, 0x34, 0x8d,
		0x24, 0x9e, 0x2a, 0x90, 0xc8, 0x8f, 0x31, 0xf9, 0x94, 0x8b,
		0xb6, 0x5d, 0x52, 0x07,
	    },
	    {
		/* x is zero, y is the result of the sqrt ladder; also on the
		   curve for y^2 = x^3 - 7. */
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x8f, 0x53, 0x7e, 0xef, 0xdf, 0xc1, 0x60, 0x6a,
		0x07, 0x27, 0xcd, 0x69, 0xb4, 0xa7, 0x33, 0x3d, 0x38, 0xed,
		0x44, 0xe3, 0x93, 0x2a, 0x71, 0x79, 0xee, 0xcb, 0x4b, 0x6f,
		0xba, 0x93, 0x60, 0xdc,
	    },
	    {/* x is zero, y is the result of the sqrt ladder; also on the curve
		for y^2 = x^3 - 7. */
	     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x70,
	     0xac, 0x81, 0x10, 0x20, 0x3e, 0x9f, 0x95, 0xf8, 0xd8, 0x32, 0x96,
	     0x4b, 0x58, 0xcc, 0xc2, 0xc7, 0x12, 0xbb, 0x1c, 0x6c, 0xd5, 0x8e,
	     0x86, 0x11, 0x34, 0xb4, 0x8f, 0x45, 0x6c, 0x9b, 0x53}};
	const unsigned char pubkeyc[66] = {
	    ...
	    0x54, 0x19, 0x9C, 0x47, 0xD0, 0x8F, 0xFB, 0x10, 0xD4, 0xB8, 0x00};
	...
	for (i = 0; i < 256; i++) {
		...
		VG_UNDEF(&pubkey, sizeof(pubkey));
		CHECK(secp256k1_ec_pubkey_parse(ctx, &pubkey, shortkey, 1) ==
		      0);
		VG_CHECK(&pubkey, sizeof(pubkey));
		CHECK(ecount == 0);
		CHECK(secp256k1_pubkey_load(ctx, &ge, &pubkey) == 0);
	...
	for (i = 0; i < 65536; i++) {
		...
		shortkey[1] = i >> 8;
		VG_UNDEF(&pubkey, sizeof(pubkey));
		CHECK(secp256k1_ec_pubkey_parse(ctx, &pubkey, shortkey, 2) ==
		      0);
		VG_CHECK(&pubkey, sizeof(pubkey));
		CHECK(ecount == 0);
		CHECK(secp256k1_pubkey_load(ctx, &ge, &pubkey) == 0);
	...
	ecount = 0;
	VG_UNDEF(&pubkey, sizeof(pubkey));
	/* 33 bytes claimed on otherwise valid input starting with 0x04, fail,
	 * zeroize output, no illegal arg error. */
	CHECK(secp256k1_ec_pubkey_parse(ctx, &pubkey, pubkeyc, 33) == 0);
	VG_CHECK(&pubkey, sizeof(pubkey));
	CHECK(ecount == 0);
	CHECK(secp256k1_pubkey_load(ctx, &ge, &pubkey) == 0);
	CHECK(ecount == 1);
	/* NULL pubkey, illegal arg error. Pubkey isn't rewritten before this
	 * step, since it's NULL into the parser. */
	...
	CHECK(secp256k1_pubkey_load(ctx, &ge, &pubkey) == 0);
	CHECK(ecount == 2);
	/* 64 bytes claimed on input starting with 0x04, fail, zeroize output,
	 * no illegal arg error. */
	memset(&pubkey, 0xfe, sizeof(pubkey));
	ecount = 0;
	VG_UNDEF(&pubkey, sizeof(pubkey));
	...
	CHECK(secp256k1_ec_pubkey_parse(secp256k1_context_no_precomp, &pubkey,
					pubkeyc, 65) == 1);
	VG_CHECK(&pubkey, sizeof(pubkey));
	CHECK(ecount == 0);
	...
	ecount = 0;
	len = 65;
	CHECK(secp256k1_ec_pubkey_serialize(ctx, NULL, &len, &pubkey,
					    SECP256K1_EC_UNCOMPRESSED) == 0);
	CHECK(ecount == 1);
	CHECK(len == 0);
	CHECK(secp256k1_ec_pubkey_serialize(ctx, sout, NULL, &pubkey,
					    SECP256K1_EC_UNCOMPRESSED) == 0);
	CHECK(ecount == 2);
	len = 65;
	VG_UNDEF(sout, 65);
	CHECK(secp256k1_ec_pubkey_serialize(ctx, sout, &len, NULL,
					    SECP256K1_EC_UNCOMPRESSED) == 0);
	VG_CHECK(sout, 65);
	CHECK(ecount == 3);
	CHECK(len == 0);
	...
	len = 65;
	VG_UNDEF(sout, 65);
	CHECK(secp256k1_ec_pubkey_serialize(ctx, sout, &len, &pubkey,
	...
	secp256k1_context_set_illegal_callback(
	    ctx, uncounting_illegal_callback_fn, &ecount2);
	CHECK(secp256k1_ec_pubkey_parse(ctx, NULL, NULL, 65) == 0);
	CHECK(ecount == 1);
	CHECK(ecount2 == 10);
	secp256k1_context_set_illegal_callback(ctx, NULL, NULL);
...
void run_eckey_edge_case_test(void) {
	...
	CHECK(ecount == 3);
	len = 33;
	CHECK(secp256k1_ec_pubkey_serialize(ctx, ctmp, &len, &pubkey,
					    SECP256K1_EC_COMPRESSED) == 1);
	CHECK(secp256k1_ec_pubkey_serialize(ctx, ctmp2, &len, &pubkey_negone,
					    SECP256K1_EC_COMPRESSED) == 1);
	...
	CHECK(secp256k1_ec_pubkey_serialize(ctx, ctmp, &len, &pubkey,
					    SECP256K1_EC_COMPRESSED) == 1);
	CHECK(secp256k1_ec_pubkey_serialize(ctx, ctmp2, &len, &pubkey_one,
					    SECP256K1_EC_COMPRESSED) == 1);
	CHECK(memcmp(ctmp, ctmp2, 33) == 0);
	/* Adds to two. */
...
void random_sign(secp256k1_scalar *sigr, secp256k1_scalar *sigs,
		 const secp256k1_scalar *key, const secp256k1_scalar *msg,
		 int *recid) {
	secp256k1_scalar nonce;
	do {
		random_scalar_order_test(&nonce);
	} while (!secp256k1_ecdsa_sig_sign(&ctx->ecmult_gen_ctx, sigr, sigs,
					   key, msg, &nonce, recid));
}
...
void test_ecdsa_sign_verify(void) {
	...
	CHECK(secp256k1_ecdsa_sig_verify(&ctx->ecmult_ctx, &sigr, &sigs, &pub,
					 &msg));
	secp256k1_scalar_set_int(&one, 1);
...
/** Dummy nonce generation function that just uses a precomputed nonce, and
 * fails if it is not accepted. Use only for testing. */
...
static int nonce_function_test_fail(unsigned char *nonce32,
				    ...
				    const unsigned char *algo16, void *data,
				    unsigned int counter) {
	/* Dummy nonce generator that has a fatal error on the first counter
	 * value. */
	if (counter == 0) {
		return 0;
	}
	return nonce_function_rfc6979(nonce32, msg32, key32, algo16, data,
				      counter - 1);
}
...
static int nonce_function_test_retry(unsigned char *nonce32,
				     ...
				     const unsigned char *algo16, void *data,
				     unsigned int counter) {
	/* Dummy nonce generator that produces unacceptable nonces for the first
	 * several counter values. */
	if (counter < 3) {
	...
	/* Retry rate of 6979 is negligible esp. as we only call this in
	 * deterministic tests. */
	/* If someone does fine a case where it retries for secp256k1, we'd like
	 * to know. */
	if (counter > 5) {
		return 0;
	}
...
void test_ecdsa_end_to_end(void) {
	...
	CHECK(ec_privkey_export_der(ctx, seckey, &seckeylen, privkey,
				    secp256k1_rand_bits(1) == 1));
	...
	CHECK(secp256k1_ecdsa_sign(ctx, &signature[0], message, privkey, NULL,
				   NULL) == 1);
	CHECK(secp256k1_ecdsa_sign(ctx, &signature[4], message, privkey, NULL,
				   NULL) == 1);
	CHECK(secp256k1_ecdsa_sign(ctx, &signature[1], message, privkey, NULL,
				   extra) == 1);
	extra[31] = 1;
	CHECK(secp256k1_ecdsa_sign(ctx, &signature[2], message, privkey, NULL,
				   extra) == 1);
	extra[31] = 0;
	extra[0] = 1;
	CHECK(secp256k1_ecdsa_sign(ctx, &signature[3], message, privkey, NULL,
				   extra) == 1);
	...
	CHECK(secp256k1_ecdsa_verify(ctx, &signature[3], message, &pubkey) ==
	      1);
	/* Test lower-S form, malleate, verify and fail, test again, malleate
	 * again */
	...
	CHECK(secp256k1_ecdsa_signature_normalize(ctx, &signature[5],
						  &signature[5]));
	CHECK(!secp256k1_ecdsa_signature_normalize(ctx, NULL, &signature[5]));
	CHECK(!secp256k1_ecdsa_signature_normalize(ctx, &signature[5],
						   &signature[5]));
	...
	CHECK(secp256k1_ecdsa_signature_serialize_der(ctx, sig, &siglen,
						      &signature[0]) == 1);
	memset(&signature[0], 0, sizeof(signature[0]));
	CHECK(secp256k1_ecdsa_signature_parse_der(ctx, &signature[0], sig,
						  siglen) == 1);
	CHECK(secp256k1_ecdsa_verify(ctx, &signature[0], message, &pubkey) ==
	      1);
...
void test_random_pubkeys(void) {
	...
	if (len == 65) {
		in[0] = secp256k1_rand_bits(1)
			    ? 4
			    : (secp256k1_rand_bits(1) ? 6 : 7);
	} else {
	...
	if (secp256k1_eckey_pubkey_parse(&elem, in, len)) {
		...
		CHECK(secp256k1_eckey_pubkey_serialize(&elem, out, &size,
						       len == 33));
		CHECK(size == len);
		...
		if (res) {
			ge_equals_ge(&elem, &elem2);
			CHECK(secp256k1_eckey_pubkey_serialize(&elem, out,
							       &size, 0));
...
int test_ecdsa_der_parse(const unsigned char *sig, size_t siglen,
			 int certainly_der, int certainly_not_der) {
	...
	parsed_der =
	    secp256k1_ecdsa_signature_parse_der(ctx, &sig_der, sig, siglen);
	if (parsed_der) {
		ret |= (!secp256k1_ecdsa_signature_serialize_compact(
			   ctx, compact_der, &sig_der))
		       << 0;
		valid_der = (memcmp(compact_der, zeroes, 32) != 0) &&
			    (memcmp(compact_der + 32, zeroes, 32) != 0);
	}
	if (valid_der) {
		ret |= (!secp256k1_ecdsa_signature_serialize_der(
			   ctx, roundtrip_der, &len_der, &sig_der))
		       << 1;
		roundtrips_der = (len_der == siglen) &&
				 memcmp(roundtrip_der, sig, siglen) == 0;
	}

	parsed_der_lax =
	    ecdsa_signature_parse_der_lax(ctx, &sig_der_lax, sig, siglen);
	if (parsed_der_lax) {
		ret |= (!secp256k1_ecdsa_signature_serialize_compact(
			   ctx, compact_der_lax, &sig_der_lax))
		       << 10;
		valid_der_lax = (memcmp(compact_der_lax, zeroes, 32) != 0) &&
				(memcmp(compact_der_lax + 32, zeroes, 32) != 0);
	}
	if (valid_der_lax) {
		ret |= (!secp256k1_ecdsa_signature_serialize_der(
			   ctx, roundtrip_der_lax, &len_der_lax, &sig_der_lax))
		       << 11;
		roundtrips_der_lax =
		    (len_der_lax == siglen) &&
		    memcmp(roundtrip_der_lax, sig, siglen) == 0;
	}
	...
	if (valid_der) {
		...
		ret |= (memcmp(roundtrip_der_lax, roundtrip_der, len_der) != 0)
		       << 14;
	}
	...
	if (parsed_openssl) {
		...
		valid_openssl = !BN_is_negative(r) && !BN_is_negative(s) &&
				BN_num_bits(r) > 0 && BN_num_bits(r) <= 256 &&
				BN_num_bits(s) > 0 && BN_num_bits(s) <= 256;
		if (valid_openssl) {
	...
	if (len_openssl <= 2048) {
		...
		roundtrips_openssl =
		    valid_openssl && ((size_t)len_openssl == siglen) &&
		    (memcmp(roundtrip_openssl, sig, siglen) == 0);
	} else {
		len_openssl = 0;
	}
	...
	if (roundtrips_openssl) {
		...
		ret |= (memcmp(roundtrip_der, roundtrip_openssl, len_der) != 0)
		       << 9;
	}
#endif
	return ret;
...
static void random_ber_signature(unsigned char *sig, size_t *len,
	...
	*certainly_not_der = 0;
	...
	for (n = 0; n < 2; n++) {
		/* We generate two classes of numbers: nlow==1 "low" ones (up to
		 * 32 bytes), nlow==0 "high" ones (32 bytes with 129 top bits
		 * set, or larger than 32 bytes) */
		nlow[n] = der ? 1 : (secp256k1_rand_bits(3) != 0);
		/* The length of the number in bytes (the first byte of which
		 * will always be nonzero) */
		nlen[n] = nlow[n] ? secp256k1_rand_int(33)
				  : 32 + secp256k1_rand_int(200) *
					     secp256k1_rand_int(8) / 8;
		CHECK(nlen[n] <= 232);
		/* The top bit of the number. */
		nhbit[n] = (nlow[n] == 0 && nlen[n] == 32)
			       ? 1
			       : (nlen[n] == 0 ? 0 : secp256k1_rand_bits(1));
		/* The top byte of the number (after the potential hardcoded 16
		 * 0xFF characters for "high" 32 bytes numbers) */
		nhbyte[n] = nlen[n] == 0
				? 0
				: (nhbit[n] ? 128 + secp256k1_rand_bits(7)
					    : 1 + secp256k1_rand_int(127));
		/* The number of zero bytes in front of the number (which is 0
		 * or 1 in case of DER, otherwise we extend up to 300 bytes) */
		nzlen[n] = der ? ((nlen[n] == 0 || nhbit[n]) ? 1 : 0)
			       : (nlow[n] ? secp256k1_rand_int(3)
		...
		/* The length of the length descriptor for the number. 0 means
		 * short encoding, anything else is long encoding. */
		nlenlen[n] = nlen[n] + nzlen[n] < 128
				 ? 0
				 : (nlen[n] + nzlen[n] < 256 ? 1 : 2);
		if (!der) {
			/* nlenlen[n] max 127 bytes */
			int add = secp256k1_rand_int(127 - nlenlen[n]) *
				  secp256k1_rand_int(16) *
				  secp256k1_rand_int(16) / 256;
			nlenlen[n] += add;
			if (add != 0) {
				*certainly_not_der = 1;
			}
	...
	tlen = 2 + nlenlen[0] + nlen[0] + nzlen[0] + 2 + nlenlen[1] + nlen[1] +
	       nzlen[1];
	CHECK(tlen <= 856);
	...
	elen = (der || indet)
		   ? 0
		   : secp256k1_rand_int(980 - tlen) * secp256k1_rand_int(8) / 8;
	if (elen != 0) {
		*certainly_not_der = 1;
	}
	tlen += elen;
	...
	glen = der ? 0
		   : secp256k1_rand_int(990 - tlen) * secp256k1_rand_int(8) / 8;
	if (glen != 0) {
		*certainly_not_der = 1;
	}
	CHECK(tlen + glen <= 990);
	...
	if (indet) {
		...
		*certainly_not_der = 1;
		...
		if (!der) {
			int add = secp256k1_rand_int(127 - tlenlen) *
				  secp256k1_rand_int(16) *
				  secp256k1_rand_int(16) / 256;
			tlenlen += add;
			if (add != 0) {
	...
	for (n = 0; n < 2; n++) {
		...
		if (nlenlen[n] == 0) {
			...
			assign_big_endian(sig + *len, nlenlen[n],
					  nlen[n] + nzlen[n]);
			*len += nlenlen[n];
		}
		/* Write zero padding */
...
void run_ecdsa_der_parse(void) {
	...
	for (i = 0; i < 200 * count; i++) {
		...
		random_ber_signature(buffer, &buflen, &certainly_der,
				     &certainly_not_der);
		CHECK(buflen <= 2048);
		for (j = 0; j < 16; j++) {
			int ret = 0;
			if (j > 0) {
				...
				/* We don't know anything anymore about the
				 * DERness of the result */
				certainly_der = 0;
				certainly_not_der = 0;
			}
			ret = test_ecdsa_der_parse(
...
void test_ecdsa_edge_cases(void) {
	...
	/* Test the case where ECDSA recomputes a point that is infinity. */
	{
		...
		msg = ss;
		CHECK(secp256k1_ecdsa_sig_verify(&ctx->ecmult_ctx, &sr, &ss,
						 &key, &msg) == 0);
	}
	...
	/* Verify signature with r of zero fails. */
	{
		const unsigned char pubkey_mods_zero[33] = {
		    ...
		    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe, 0xba,
		    0xae, 0xdc, 0xe6, 0xaf, 0x48, 0xa0, 0x3b, 0xbf, 0xd2,
	...
	{
		const unsigned char pubkey[33] = {
		    0x02, 0x14, 0x4e, 0x5a, 0x58, 0xef, 0x5b, 0x22, 0x6f,
		    0xd2, 0xe2, 0x07, 0x6a, 0x77, 0xcf, 0x05, 0xb4, 0x1d,
		    0xe7, 0x4a, 0x30, 0x98, 0x27, 0x8c, 0x93, 0xe6, 0xe6,
		    0x3c, 0x0b, 0xc4, 0x73, 0x76, 0x25};
		const unsigned char pubkey2[33] = {
		    0x02, 0x8a, 0xd5, 0x37, 0xed, 0x73, 0xd9, 0x40, 0x1d,
		    0xa0, 0x33, 0xd2, 0xdc, 0xf0, 0xaf, 0xae, 0x34, 0xcf,
		    0x5f, 0x96, 0x4c, 0x73, 0x28, 0x0f, 0x92, 0xc0, 0xf6,
		    0x9d, 0xd9, 0xb2, 0x09, 0x10, 0x62};
	...
	{
		const unsigned char pubkey[33] = {
		    0x03, 0xaf, 0x97, 0xff, 0x7d, 0x3a, 0xf6, 0xa0, 0x02,
		    0x94, 0xbd, 0x9f, 0x4b, 0x2e, 0xd7, 0x52, 0x28, 0xdb,
		    0x49, 0x2a, 0x65, 0xcb, 0x1e, 0x27, 0x57, 0x9c, 0xba,
		    0x74, 0x20, 0xd5, 0x1d, 0x20, 0xf1};
		const unsigned char csr[32] = {
		    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
		    0x45, 0x51, 0x23, 0x19, 0x50, 0xb7, 0x5f, 0xc4,
	...
	/* Signature where s would be zero. */
	{
		...
		secp256k1_context_set_illegal_callback(
		    ctx, counting_illegal_callback_fn, &ecount);
		CHECK(secp256k1_ecdsa_sign(ctx, &sig, msg, key,
		...
		CHECK(secp256k1_ecdsa_sign(ctx, &sig, msg, key,
					   ...
					   nonce) == 1);
		CHECK(ecount == 0);
		CHECK(secp256k1_ecdsa_sign(ctx, NULL, msg, key,
					   precomputed_nonce_function,
					   nonce2) == 0);
		CHECK(ecount == 1);
		CHECK(secp256k1_ecdsa_sign(ctx, &sig, NULL, key,
					   precomputed_nonce_function,
					   nonce2) == 0);
		CHECK(ecount == 2);
		CHECK(secp256k1_ecdsa_sign(ctx, &sig, msg, NULL,
					   precomputed_nonce_function,
					   nonce2) == 0);
		CHECK(ecount == 3);
		CHECK(secp256k1_ecdsa_sign(ctx, &sig, msg, key,
		...
		CHECK(secp256k1_ec_pubkey_create(ctx, &pubkey, NULL) == 0);
		CHECK(ecount == 7);
		/* That pubkeyload fails via an ARGCHECK is a little odd but
		 * makes sense because pubkeys are an opaque data type. */
		CHECK(secp256k1_ecdsa_verify(ctx, &sig, msg, &pubkey) == 0);
		CHECK(ecount == 8);
		siglen = 72;
		CHECK(secp256k1_ecdsa_signature_serialize_der(
			  ctx, NULL, &siglen, &sig) == 0);
		CHECK(ecount == 9);
		CHECK(secp256k1_ecdsa_signature_serialize_der(ctx, signature,
							      NULL, &sig) == 0);
		CHECK(ecount == 10);
		CHECK(secp256k1_ecdsa_signature_serialize_der(
			  ctx, signature, &siglen, NULL) == 0);
		CHECK(ecount == 11);
		CHECK(secp256k1_ecdsa_signature_serialize_der(
			  ctx, signature, &siglen, &sig) == 1);
		CHECK(ecount == 11);
		CHECK(secp256k1_ecdsa_signature_parse_der(ctx, NULL, signature,
							  siglen) == 0);
		CHECK(ecount == 12);
		CHECK(secp256k1_ecdsa_signature_parse_der(ctx, &sig, NULL,
							  siglen) == 0);
		CHECK(ecount == 13);
		CHECK(secp256k1_ecdsa_signature_parse_der(ctx, &sig, signature,
							  siglen) == 1);
		CHECK(ecount == 13);
		siglen = 10;
		 ...
		 */
		CHECK(secp256k1_ecdsa_signature_serialize_der(
			  ctx, signature, &siglen, &sig) == 0);
		CHECK(ecount == 13);
		ecount = 0;
		CHECK(secp256k1_ecdsa_signature_normalize(ctx, NULL, NULL) ==
		      0);
		CHECK(ecount == 1);
		CHECK(secp256k1_ecdsa_signature_serialize_compact(ctx, NULL,
								  &sig) == 0);
		CHECK(ecount == 2);
		CHECK(secp256k1_ecdsa_signature_serialize_compact(
			  ctx, signature, NULL) == 0);
		CHECK(ecount == 3);
		CHECK(secp256k1_ecdsa_signature_serialize_compact(
			  ctx, signature, &sig) == 1);
		CHECK(ecount == 3);
		CHECK(secp256k1_ecdsa_signature_parse_compact(ctx, NULL,
							      signature) == 0);
		CHECK(ecount == 4);
		CHECK(secp256k1_ecdsa_signature_parse_compact(ctx, &sig,
							      NULL) == 0);
		CHECK(ecount == 5);
		CHECK(secp256k1_ecdsa_signature_parse_compact(ctx, &sig,
							      signature) == 1);
	...
	for (t = 0; t < 2; t++) {
		...
		memset(key, 0xFF, 32);
		CHECK(secp256k1_ecdsa_sign(ctx, &sig, msg, key, NULL, extra) ==
		      0);
		...
		CHECK(secp256k1_ecdsa_sign(ctx, &sig, msg, key,
					   ...
					   extra) == 0);
		CHECK(is_empty_signature(&sig));
		/* The retry loop successfully makes its way to the first good
		 * value. */
		CHECK(secp256k1_ecdsa_sign(ctx, &sig, msg, key,
					   nonce_function_test_retry,
					   extra) == 1);
		CHECK(!is_empty_signature(&sig));
		CHECK(secp256k1_ecdsa_sign(ctx, &sig2, msg, key,
					   nonce_function_rfc6979, extra) == 1);
		...
		CHECK(!is_empty_signature(&sig2));
		CHECK(memcmp(&sig, &sig2, sizeof(sig)) == 0);
		/* The default nonce function changes output with different
		 * messages. */
		for (i = 0; i < 256; i++) {
			int j;
			msg[0] = i;
			CHECK(secp256k1_ecdsa_sign(ctx, &sig2, msg, key, NULL,
						   extra) == 1);
			CHECK(!is_empty_signature(&sig2));
	...
	{
		/* Check that optional nonce arguments do not have equivalent
		 * effect. */
		...
		VG_UNDEF(nonce3, 32);
		VG_UNDEF(nonce4, 32);
		CHECK(nonce_function_rfc6979(nonce, zeros, zeros, NULL, NULL,
					     0) == 1);
		VG_CHECK(nonce, 32);
		CHECK(nonce_function_rfc6979(nonce2, zeros, zeros, zeros, NULL,
					     0) == 1);
		VG_CHECK(nonce2, 32);
		CHECK(nonce_function_rfc6979(nonce3, zeros, zeros, NULL,
					     (void *)zeros, 0) == 1);
		VG_CHECK(nonce3, 32);
		CHECK(nonce_function_rfc6979(nonce4, zeros, zeros, zeros,
					     (void *)zeros, 0) == 1);
		VG_CHECK(nonce4, 32);
...
void test_ecdsa_openssl(void) {
	...
	CHECK(ec_key != NULL);
	CHECK(ECDSA_sign(0, message, sizeof(message), signature, &sigsize,
			 ec_key));
	CHECK(secp256k1_ecdsa_sig_parse(&sigr, &sigs, signature, sigsize));
	CHECK(secp256k1_ecdsa_sig_verify(&ctx->ecmult_ctx, &sigr, &sigs, &q,
					 &msg));
	secp256k1_scalar_set_int(&one, 1);
	...
	CHECK(secp256k1_ecdsa_sig_serialize(signature, &secp_sigsize, &sigr,
					    &sigs));
	CHECK(ECDSA_verify(0, message, sizeof(message), signature, secp_sigsize,
			   ec_key) == 1);
...
int main(int argc, char **argv) {
	...
	if (argc > 2) {
		...
		if ((frand == NULL) || fread(&seed16, sizeof(seed16), 1,
					     frand) != sizeof(seed16)) {
	...
	printf(
	    ...
	    "02x\n",
	    seed16[0], seed16[1], seed16[2], seed16[3], seed16[4], seed16[5],
	    seed16[6], seed16[7], seed16[8], seed16[9], seed16[10], seed16[11],
	...
	if (secp256k1_rand_bits(1)) {
		...
		CHECK(secp256k1_context_randomize(
		    ctx, secp256k1_rand_bits(1) ? run32 : NULL));
	}
	...
	printf(
	    ...
	    "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%"
	    "02x\n",
	    run32[0], run32[1], run32[2], run32[3], run32[4], run32[5],
	    run32[6], run32[7], run32[8], run32[9], run32[10], run32[11],
