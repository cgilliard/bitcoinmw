...
unsigned long long aes_context_size() { return sizeof(struct AESContext); }
...
static void KeyExpansion(unsigned char *RoundKey, const unsigned char *Key) {
	...
	for (i = Nk; i < Nb * (Nr + 1); ++i) {
		...
		if (i % Nk == 0) {
			// This function shifts the 4 unsigned chars in a word
			// to the left once. [a0,a1,a2,a3] becomes [a1,a2,a3,a0]
			...
			// SubWord() is a function that takes a four-unsigned
			// char input word and applies the S-box to each of the
			// four unsigned chars to produce an output word.
