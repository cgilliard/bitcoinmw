...
static const byte gf16_mult[16][16] = {
...
static const byte gf16_inv[16] = {0,  1, 9,  14, 13, 11, 7, 6,
				  15, 2, 12, 5,	 10, 4,	 3, 8};

static int is_full_rank(const uint16 matrix[64][64]) {
	byte temp[64][64];
	for (int i = 0; i < 64; ++i) {
		for (int j = 0; j < 64; ++j) {
	...
	for (int i = 0; i < 64; ++i) {
		...
		byte pivot_val = temp[i][i];
		byte inv = gf16_inv[pivot_val];
		for (int j = 0; j < 64; ++j) {
			temp[i][j] = gf16_mult[temp[i][j]][inv];
		}
		for (int k = 0; k < 64; ++k) {
			if (k != i && temp[k][i] != 0) {
				byte factor = temp[k][i];
				for (int j = 0; j < 64; ++j) {
					temp[k][j] ^=
...
void generate_matrix(uint16 matrix[64][64], struct AESContext* aes) {
	set_bytes((byte*)matrix, 0, 64 * 64 * sizeof(uint16));
	do {
		for (int i = 0; i < 64; ++i) {
