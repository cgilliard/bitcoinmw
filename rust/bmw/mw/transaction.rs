...
use crypto::signature::Message;
use mw::errors::*;
use mw::kernel::Kernel;
use prelude::*;
use misc::{slice_copy, subslice_mut};
use util::rbtree::{RbTree, RbTreeNode};
...
impl Transaction {
	...
	pub fn merge(&mut self, ctx: &Ctx, t: Transaction) -> Result<()> {
	...
	pub fn add_input(&mut self, input: Commitment) -> Result<()> {
		self.inputs.push(input)
	}

	pub fn add_output(&mut self, output: Commitment, proof: RangeProof) -> Result<()> {
		self.outputs.push((output, proof))
	}

	pub fn add_kernel(&mut self, kernel: Kernel) -> Result<()> {
		let ptr = RbTreeNode::alloc(kernel.clone())?;
	...
	pub fn kernel_merkle_root(&self) -> Result<Message> {
	...
	pub fn validate(&self, ctx: &Ctx, overage: u64) -> Result<()> {
	...
	fn append_kernels(&mut self, ctx: &Ctx, node: Ptr<RbTreeNode<Kernel>>) -> Result<()> {
		...
		let ptr = RbTreeNode::alloc((*node.value).clone())?;
		match self.kernels.insert(ptr) {
			Some(duplicate) => {
				duplicate.release();
				err!(Duplicate)
...
mod test {
	...
	#[test]
	fn test_kernel_merkle_roots() -> Result<()> {
		let ctx = Ctx::new()?;
		let mut tx = Transaction::empty();
