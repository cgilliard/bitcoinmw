...
macro_rules! box_slice {
	($value:expr, $len:expr) => {{
		...
		use core::ptr::{null_mut, slice_from_raw_parts_mut, write};
		let count = $len;
		...
		Box {
			ptr: Ptr::new(slice_from_raw_parts_mut(ptr as *mut _, count)),
		}
	}};
}
...
macro_rules! try_box_slice {
	($value:expr, $len:expr) => {{
		use core::mem::size_of_val;
		use core::ptr::{slice_from_raw_parts_mut, write};
		let count = $len;
		let elem_size = size_of_val(&$value);
		let total_size = elem_size * count;
		if total_size == 0 {
			...
			unsafe {
				...
				if rptr.is_null() {
					...
					Ok(Box {
						ptr: Ptr::new(slice_from_raw_parts_mut(rptr as *mut _, count)),
					})
				}
			}
		}
