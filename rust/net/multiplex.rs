...
use net::ffi::{
	...
	multiplex_register, multiplex_size, multiplex_wait,
};
use net::Socket;
use prelude::*;
...
mod test {
	...
	fn test_multiplex1() -> Result<(), Error> {
		...
		assert_eq!(m1.wait(&mut events, Some(0))?, 0);
	...
	fn test_multiple_events() -> Result<(), Error> {
		...
		assert_eq!(m1.wait(&mut events, Some(0))?, 1);

		let r1 = events[0].socket().accept()?;
		let r2 = events[0].socket().accept()?;
		let r3 = events[0].socket().accept()?;
		...
		loop {
			unsafe {
				let evts = m1.wait(&mut events, Some(0))? as usize;
				for i in 0..evts {
