...
pub fn murmur3_128_of_u64(source: u64, seed: u32) -> Result<u128, Error> {
	let slice = unsafe { from_raw_parts(&source as *const u64 as *const u8, size_of::<u64>()) };
	murmur3_x64_128_of_slice(slice, seed)
}

pub fn murmur3_x64_128_of_slice(source: &[u8], seed: u32) -> Result<u128, Error> {
	...
	loop {
		match min(buf.len(), 16) {
			...
			0 => {
				...
				h1 = h1.wrapping_add(h2);
				h2 = h2.wrapping_add(h1);
				return Ok(((h2 as u128) << 64) | (h1 as u128));
			}
			_ => {
				let read = buf.len();
				processed += read;
