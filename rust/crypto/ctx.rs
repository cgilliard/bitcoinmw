...
use crypto::sha3::Sha3ByteSize::Sha3_256;
...
impl Ctx {
	...
	pub fn secp(&self) -> *mut Secp256k1Context {
		self.secp
	}
	...
	pub fn hash_kernel(
		...
		self.sha3.reset();
		self.sha3.update(excess.as_ref()); // 33 bytes
	...
	pub fn commit(&self, v: u64, blind: &SecretKey) -> Result<Commitment, Error> {
		let mut uncomp = CommitmentUncompressed::new([0u8; 64]);
		let res = unsafe {
		...
		if res != 1 {
			...
			let mut ret = Commitment::new([0u8; 33]);
			let res = unsafe {
	...
	pub fn blind_sum(
		...
		let mut blind_out = SecretKey::new([0u8; 32]);
	...
	pub fn verify_range_proof(&self, commit: &Commitment, proof: &RangeProof) -> Result<(), Error> {
...
mod test {
	...
	fn test_mimblewimble_tx() {
		...
		let nonce1 = SecretKey::gen(&mut secp);
		...
		let nonce2 = SecretKey::gen(&mut secp);
		...
		secp.verify_balance(&[&input1], &[&change, &output1, &excess], 0)
			.unwrap();
		secp.verify_single(&aggsig, &msg, &nonce_sum, &pubkey_sum, &pubkey_sum, false)
			.unwrap();
	}
	...
	fn test_mimblewimble_interactive_with_fee() -> Result<(), Error> {
		...
		let blind2 = SecretKey::gen(&mut secp_send);
		let _change = secp_send.commit(100 - fee, &blind2)?; // Fee deducted from change (95)
		...
		let blind4 = SecretKey::gen(&mut secp_recv);
		let _output2 = secp_recv.commit(199, &blind4)?; // Full amount, no fee here
