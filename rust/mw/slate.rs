...
impl Slate {
	...
	pub fn finalize(&mut self, ctx: &Ctx) -> Result<Transaction> {
		if self.pdata.len() == 0 {
	...
	fn excess_commit_sum(&self, ctx: &Ctx) -> Result<Commitment> {
	...
	fn pub_nonce_sum(&self, ctx: &Ctx) -> Result<PublicKey> {
		if self.pdata.len() == 0 {
			return err!(IllegalState);
		}
	...
	fn pub_blind_sum(&self, ctx: &Ctx) -> Result<PublicKey> {
...
mod test {
	...
	#[test]
	fn test_slate_with_keychain() -> Result<()> {
