...
impl KeyChain {
	...
	pub fn derive_key(&self, ctx: &Ctx, path: &[u64; 2]) -> SecretKey {
		...
		let mut skey = SecretKey::new([0u8; 32]);
		loop {
			unsafe {
				AES_CTR_xcrypt_buffer(self.aes256_ctx, skey.as_mut_ptr() as *mut u8, 32);
			}
...
mod test {
	...
	fn test_keychain() -> Result<(), Error> {
		...
		assert!(sk1.as_ref() != sk2.as_ref());
		assert!(sk2.as_ref() != sk3.as_ref());
		assert_eq!(sk2.as_ref(), sk4.as_ref());
